...
Finished images go to **`/opt/data/media/out/`** only (not `/opt/data/*.png`, not `./outputs`). See skill **`image-gen`**. Pass `--output-dir /opt/data/media/out` to `run_workflow.py` / `run_batch.py`.
