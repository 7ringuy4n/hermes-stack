...
**Stack path:** router-worker `POST /v1/chat/completions` requested model **`vision-ocr`** (via `architect/lib/vision_ocr.py` in ingest, jobs, dispatcher, Zalo host). Router Worker prefers the OmniRoute combo and may use only an operator-declared vision-capable fallback model.
