...
10. Visual weather/info PDF: finish with styled office-file (TITLE/ICON/facts) via **`file-gen`**. Never ask for image API keys or show session-restore / numbered recovery menus when Omni image-gen fails. After web_search for a PDF ask, the next tool call must be office-file — never stop at a chat weather summary.
