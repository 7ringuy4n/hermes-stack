...
9. Chat PDF/DOCX/XLSX create-and-send: **you** author the full file body (structure and content), then skill **`file-gen`** → Dispatcher `/v1/office-file` only. Never `skill_view` ambiguous `pdf`/`docx`/`xlsx`, never `pip`/`uv` install reportlab/pypdf, never narrate library installs.
