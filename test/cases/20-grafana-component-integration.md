...
| 9Router (always on) | TCP `assistant_service_up{service="9router"}` + `n9router_scrape_success` | Grafana/Prometheus → `nine-exporter` |
| OmniRouter (`ENABLE_OMNIROUTER=1`) | `assistant_service_up{service="omni-router"}` + `omnirouter_scrape_success` | OmniRouter **and** Grafana/Prometheus |
...
5. Record `n9router_scrape_success` (always when Grafana on).
6. If OmniRouter on: `omnirouter_scrape_success==1`. If off: omni-exporter job may be absent — **PASS**.
...
- Expected `assistant_service_up==1` (including **9router** TCP)
- 9Router authenticated scrape success when Grafana on
- OmniRouter scrape success **only if** that flag is 1

## Fail events

- Grafana on but Prometheus target `stack-exporter` down
- Deployed OmniRouter with `assistant_service_up{service="omni-router"}==0` (wrong `/health` path)
- 9Router container up but TCP probe 0
- Dashboard Hardware panels empty because `node-exporter` not paired

## Local unit (no VPS)

`python test/scripts/grafana_pairing_unit.py` — compose `HEALTH_TARGETS` includes `9router`; Prometheus scrape jobs list nine/omni/node/stack exporters.
