...
| OmniRoute (always on) | TCP `assistant_service_up{service="omni-router"}` + `omnirouter_scrape_success` | Grafana/Prometheus → `omni-exporter` |
...
5. Record `omnirouter_scrape_success` (always when Grafana on).
...
- OmniRoute authenticated scrape success when Grafana on
...
`python test/scripts/grafana_pairing_unit.py` — compose `HEALTH_TARGETS` includes `omni-router`; Prometheus scrape jobs list nine/omni/node/stack exporters.
