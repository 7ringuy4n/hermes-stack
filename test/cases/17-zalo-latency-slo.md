...
3. If send is **> 5s**, inspect Hermes / router-worker / OmniRoute logs for quota (`429`, `quota`, `rate limit`) or provider fallback (`switch`, `failover`, `no healthy`, `trying next`). Those are **not** service-hang failures, but remain latency evidence.
...
- [ ] Classify hop (`classify.json` timeout, `ROUTER_WORKER_CLASSIFY_TIMEOUT_S`)
- [ ] router-worker / OmniRoute health (quota vs real hang)
