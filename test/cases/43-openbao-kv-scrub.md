...
4. **Load** — `load-openbao-env` rewrites `.env.openbao` and fills empty compose-required keys in root `.env` from KV so `up`/`update` do not start with mismatched empty secrets.
5. **Pollinations** — `POLLINATIONS_API_KEY` is a seed/scrub key (not obsolete) when present.
