...
Environment: ASSISTANT_SSH_*, ZALO_TEST_USER_ID, optional ZALO_CANCEL_MODES
(`plain,quote`), ZALO_CANCEL_DELAY_S, ZALO_CANCEL_OBSERVE_S.
"""
...
def run_mode(client, mode: str) -> str:
    remote = rf'''
set -euo pipefail
cd /opt/assistant
python3 - <<'PY'
...
delay_s={DELAY_S}
observe_s={OBSERVE_S}
tag="cancel-"+mode+"-"+str(int(time.time()))
...
if mode == "quote":
    ...
    sent=post("/send",{{"threadId":uid,"threadType":"user","text":marker}})
    ...
    quote={{"msgType":"webchat","msgId":real_id,"cliMsgId":real_id,"content":marker}}
    print("REAL_QUOTE_ID_PRESENT")

started=time.time()
request_payload={{"type":"message","payload":{{
    "threadId":uid,"threadType":"user","senderId":uid,"senderName":"test-user",
...
def main() -> int:
    ...
    try:
        for mode in MODES:
            raw=sanitize(run_mode(client, mode) or "")
            passed=("PASS_CANCEL_"+mode.upper()) in raw and "FAIL_" not in raw
            rows.append({"mode":mode,"ok":passed,"output":raw[-3000:]})
            print(raw, flush=True)
            if not passed:
                break
    finally:
        client.close()
    ok=len(rows)==len(MODES) and all(row["ok"] for row in rows)
