...
docker exec "$h2" sh -lc 'echo N9ROUTER_SET=$([ -n "$N9ROUTER_API_KEY" ] && echo 1 || echo 0); echo OPENAI_SET=$([ -n "$OPENAI_API_KEY" ] && echo 1 || echo 0); echo OPENAI_BASE=$OPENAI_BASE_URL'
set +e
docker exec "$h2" python3 -c "import os,urllib.request; k=os.environ.get('OPENAI_API_KEY') or os.environ.get('N9ROUTER_API_KEY') or ''; base=(os.environ.get('OPENAI_BASE_URL') or 'http://model-router:8096/v1').rstrip('/'); url=base+'/models'; req=urllib.request.Request(url, headers={'Authorization':'Bearer '+k});
try:
...
req2=urllib.request.Request('http://9router:20128/');
try:
 r2=urllib.request.urlopen(req2, timeout=5); print('hermes2_9router_root', r2.status)
except Exception as e:
 print('hermes2_9router_root', type(e).__name__, getattr(e,'code',e))"
set -e
echo "=== schedules ==="
