...
def _first_path(listing: str, *, suffix: str = "") -> str:
    for ln in _clean_remote(listing).splitlines():
        if not ln.startswith("/"):
            continue
        if suffix and not ln.lower().endswith(suffix):
            continue
        if "_smoke" in ln or "_overlay_crop" in ln:
            continue
        return ln
    return ""
...
def main() -> int:
    ...
    try:
        before_pptx = _clean_remote(
            sudo_bash(c, "ls -1t /data/assistant/media/out/*.pptx 2>/dev/null | head -5 || true")
        )
        before_img = _clean_remote(
            sudo_bash(
                c,
        ...
        after_pptx = _clean_remote(
            sudo_bash(c, "ls -1t /data/assistant/media/out/*.pptx 2>/dev/null | head -8 || true")
        )
        newest_pptx = _first_path(after_pptx, suffix=".pptx")
        ...
        after_img = _clean_remote(
            sudo_bash(
                c,
                "ls -1t /data/assistant/media/out/*.{jpg,jpeg,webp,png} 2>/dev/null | head -10 || true",
            )
        )
        newest_img = _first_path(after_img)
        ...
        logs = _clean_remote(
            sudo_bash(
                c,
                "docker logs --since 30m assistant-hermes-2 2>&1 | tail -n 100; "
                "docker logs --since 30m assistant-dispatcher-1 2>&1 | tail -n 50; "
                "docker logs --since 30m omni-router 2>&1 | grep -Ei 'rate.?limit|maxWaitMs|dropped' | tail -n 20 || true",
            )
        )
        ...
        pptx_ok = bool(newest_pptx) and newest_pptx not in before_pptx
        img_ok = bool(newest_img) and newest_img not in before_img
        report.update(
            {
                "pptx_ok": pptx_ok,
                "img_ok": img_ok,
