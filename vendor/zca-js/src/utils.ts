...
function aesCbcAlgorithm(key: Buffer): string {
    if (![16, 24, 32].includes(key.length)) throw new Error(`Invalid AES key length: ${key.length}`);
    return `aes-${key.length * 8}-cbc`;
}

function aesCbcEncrypt(key: Buffer, value: string): Buffer {
    const cipher = crypto.createCipheriv(aesCbcAlgorithm(key), key, Buffer.alloc(16));
    return Buffer.concat([cipher.update(value, "utf8"), cipher.final()]);
}

function aesCbcDecrypt(key: Buffer, value: Buffer): string {
    const decipher = crypto.createDecipheriv(aesCbcAlgorithm(key), key, Buffer.alloc(16));
...
export class ParamsEncryptor {
    ...
    createEncryptKey(e = 0) {
        ...
        try {
            const n = crypto.createHash("md5").update(this.zcid_ext).digest("hex").toUpperCase();
    ...
    static encodeAES(e: string, message: string, type: "hex" | "base64", uppercase: boolean, s = 0): string | null {
        ...
        try {
            {
                const encrypted = aesCbcEncrypt(Buffer.from(e, "utf8"), message).toString(type);
...
function decodeRespAES(key: string, data: string) {
    ...
    return aesCbcDecrypt(Buffer.from(key, "utf8"), Buffer.from(data, "base64"));
}
...
export function encodeAES(secretKey: string, data: string, t = 0): string | null {
    try {
        return aesCbcEncrypt(Buffer.from(secretKey, "base64"), data).toString("base64");
    } catch {
...
export function decodeAES(secretKey: string, data: string, t = 0): string | null {
    try {
        data = decodeURIComponent(data);
        return aesCbcDecrypt(Buffer.from(secretKey, "base64"), Buffer.from(data, "base64"));
    } catch {
...
export type FactoryUtils<T> = {
    ...
    encodeAES: (data: string, t?: number) => ReturnType<typeof encodeAES>;
    ...
    return <K extends (api: API, ctx: ContextSession, utils: FactoryUtils<T>) => unknown>(callback: K) => {
        return (ctx: ContextBase, api: API) => {
            ...
            const utils = {
                ...
                encodeAES(data: string, t?: number) {
