...
Protect with ADMIN_API_TOKEN (VPN Traefik or Bearer). Never returns stack traces.
...
app = FastAPI(title="assistant-admin-api", version="1.3.0")
...
def _write_admin_user(uid: str, name: str = "") -> None:
    ...
    with open(ADMIN_USERS_FILE, "w", encoding="utf-8", newline="\n") as f:
        f.write("# managed by admin-api — sole Zalo admin (exactly one)\n")
        if name.strip():
...
def _docker_bin() -> Optional[str]:
    """Host docker CLI — usually missing inside admin-api container (no docker.sock)."""
...
def _wipe_hermes_file_sessions() -> dict[str, Any]:
    """Wipe Hermes gateway state (state.db + sessions/) — Redis alone does not unbind Zalo routing."""
    removed = 0
    samples: list[str] = []

    for rel in HERMES_STATE_DB_REL:
        fp = os.path.join(HERMES_DATA, rel)
        ...
        try:
            ...
            removed += 1
            if len(samples) < 12:
                samples.append(f"state.db ({rel})")
        except OSError:
            continue

    for rel in HERMES_SESSION_DIRS:
        root = os.path.join(HERMES_DATA, rel)
    ...
    # In-container paths (HERMES_HOME may differ from admin-api mount layout)
...
def health() -> dict[str, Any]:
    ...
    return {"ok": True, "service": "admin-api", "admin_users": len(_admin_users())}
...
def _write_denied_threads(ids: set[str]) -> None:
    ...
    with open(DENIED_THREADS_FILE, "w", encoding="utf-8") as f:
        f.write("# managed by admin-api — kicked threads (overrides .env allow)\n")
...
def _write_entries(entries: list[dict[str, str]]) -> None:
    ...
    with open(ALLOWED_FILE, "w", encoding="utf-8") as f:
        f.write("# managed by admin-api — format: threadId | display name\n")
        for e in entries:
            if e.get("name"):
...
def _write_allowed_users(users: list[dict[str, str]]) -> None:
    ...
    with open(ALLOWED_USERS_FILE, "w", encoding="utf-8") as f:
        f.write("# managed by admin-api — format: uid | optional name\n")
        for u in users:
            if u.get("name"):
