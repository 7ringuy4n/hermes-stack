#!/usr/bin/env python3
"""Scrape OmniRoute dashboard usage APIs into Prometheus metrics.

Env:
  LISTEN, SCRAPE_INTERVAL
  METRIC_PREFIX   default omnirouter
  ROUTER_URL      default http://omni-router:20129
  ROUTER_PASSWORD OmniRoute dashboard password
"""
...
METRIC_PREFIX = os.environ.get("METRIC_PREFIX", "omnirouter").strip() or "omnirouter"
ROUTER_URL = (os.environ.get("ROUTER_URL") or "http://omni-router:20129").rstrip("/")
ROUTER_PASSWORD = os.environ.get("ROUTER_PASSWORD") or ""
...
def _coerce_usage(payload: Any) -> dict[str, Any]:
    """Map OmniRoute usage JSON into the flat fields Grafana panels expect."""
...
def _fetch_usage() -> dict[str, Any]:
    ...
    # Try supported OmniRoute usage endpoint variants across maintained releases.
    for path in (
        "/api/usage/history",
        "/api/usage/analytics",
        "/api/usage/stats",
