...
This size is a comfortable **core + media + security + message without Grafana/Loki/Prometheus** reference. Idle RAM on that run was roughly **~3 GiB** used with large cache; peak under dual Hermes and concurrent remote media calls is higher.

## Recommended minimums (worker base)

Sizes below assume Docker + Compose on a clean Ubuntu 24.04 host, models routed via **OmniRoute** (no large local LLM weights on disk), and data on a single SSD. **Add** the extras in [Additional usage when you enable features](#additional-usage-when-you-enable-features) if those flags/workers are on.
...
| **OmniRoute** (core default) | **`omni-exporter`** when Prometheus/Grafana is also on | **~0.4 GiB** | **~1 GB** | **~0.2 vCPU** |
